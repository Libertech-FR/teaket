...
export async function validateExtensionsList(): Promise<ExtensionsListV1[]> {
...
export async function extensionParseFile(path: string): Promise<ExtensionFileV1> {
  ...
  const yml = parse(data)
  const schema = plainToInstance(ExtensionFileV1, yml)
  return schema
}
...
export async function app() {
  try {
    if (existsSync(EXTENSIONS_FILE_PATH)) {
      ...
      const list = await validateExtensionsList()
