...
describe('SourceRequestController', () => {
  let controller: SourceRequestController
  let service: SourceRequestService
  const date = new Date()
  const _id = new Types.ObjectId()
  const { res, mockClear } = getMockRes()
  const object: SourceRequest = {
    _id,
    metadata: {
      createdAt: date,
      createdBy: 'console',
  ...
  beforeEach(async () => {
    ...
    controller = module.get<SourceRequestController>(SourceRequestController)
    service = module.get<SourceRequestService>(SourceRequestService)
  })

  describe('search', () => {
    it('should return an array of SourceRequest objects and the total count', async () => {
      const req = getMockReq()
      const query = { _id: _id.toString() }
      const limit = 10
      const skip = 1
      const expectedResult: [SourceRequest[], number] = [[object], 1]
    ...
    it('should return an array of SourceRequest objects with default pagination', async () => {
      const req = getMockReq()
      const query = { _id: _id.toString() }
      const limit = 0
      const skip = 0
  ...
  describe('read', () => {
    it('should return the data successfully', async () => {
      const expectedResult: SourceRequest = object
  ...
  describe('create', () => {
    it('should return a SourceRequest object', async () => {
      const dto: SourceRequestDto = { someProp: 'value' }
