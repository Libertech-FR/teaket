  ...
  public async findAndCount<T extends AbstractSchema | Document>(
    ...
    if (this.eventEmitter) {
      const beforeEvents = await this.eventEmitter?.emitAsync([
        ...
        'service',
        'beforeFindAndCount',
      ].join(EventEmitterSeparator), { filter, projection, options })
    ...
    if (this.eventEmitter) {
      const afterEvents = await this.eventEmitter?.emitAsync([
        ...
        'service',
        'afterFindAndCount',
      ].join(EventEmitterSeparator), { total, count })
  ...
  public async findById<T extends AbstractSchema | Document>(
    ...
    if (this.eventEmitter) {
      const beforeEvents = await this.eventEmitter?.emitAsync([
        ...
        'service',
        'beforeFindById',
      ].join(EventEmitterSeparator), { _id, projection, options })
      for (const beforeEvent of beforeEvents) {
        if (beforeEvent?.stop) throw beforeEvent?.stop
        if (beforeEvent?.projection) projection = { ...typeof projection === 'object' ? projection : {}, ...beforeEvent.projection }
        if (beforeEvent?.options) options = { ...options, ...beforeEvent.options }
    ...
    if (this.eventEmitter) {
      const afterEvents = await this.eventEmitter?.emitAsync([
        ...
        'service',
        'afterFindById',
      ].join(EventEmitterSeparator), { data })
      for (const afterEvent of afterEvents) {
  ...
  public async create<T extends AbstractSchema | Document>(data?: any, options?: SaveOptions): Promise<Document<T, any, T>> {
    ...
    if (this.eventEmitter) {
      const beforeEvents = await this.eventEmitter?.emitAsync([
        ...
        'service',
        'beforeCreate',
      ].join(EventEmitterSeparator), { data, options })
      for (const beforeEvent of beforeEvents) {
        if (beforeEvent?.stop) throw beforeEvent?.stop
    ...
    if (this.eventEmitter) {
      const afterEvents = await this.eventEmitter?.emitAsync([
        ...
        'service',
        'afterFindAndCount',
      ].join(EventEmitterSeparator), { created })
      for (const afterEvent of afterEvents) {
  ...
  public async update<T extends AbstractSchema | Document>(
    ...
    if (this.eventEmitter) {
      const beforeEvents = await this.eventEmitter?.emitAsync([
        ...
        'service',
        'beforeUpdate',
      ].join(EventEmitterSeparator), { _id, update, options })
      for (const beforeEvent of beforeEvents) {
        if (beforeEvent?.stop) throw beforeEvent?.stop
    ...
    if (this.eventEmitter) {
      const afterEvents = await this.eventEmitter?.emitAsync([
        ...
        'service',
        'afterFindAndCount',
      ].join(EventEmitterSeparator), { updated })
      for (const afterEvent of afterEvents) {
  ...
  public async delete<T extends AbstractSchema | Document>(
    ...
    if (this.eventEmitter) {
      const beforeEvents = await this.eventEmitter?.emitAsync([
        ...
        'service',
        'beforeDelete',
      ].join(EventEmitterSeparator), { _id, options })
      for (const beforeEvent of beforeEvents) {
        if (beforeEvent?.stop) throw beforeEvent?.stop
        if (beforeEvent?.options) options = { ...options, ...beforeEvent.options }
      }
    ...
    if (this.eventEmitter) {
      const afterEvents = await this.eventEmitter?.emitAsync([
        ...
        'service',
        'afterFindAndCount',
      ].join(EventEmitterSeparator), { deleted })
      for (const afterEvent of afterEvents) {
